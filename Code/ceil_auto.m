...
drive = '/Cronyn/';
directory = strcat(drive,s_yr,'/',s_mth,'/',s_dy,'/','*.nc');
...
for j = 3:N 
      file = ncfile(j).name;
      densitynew = ncread(file,'beta_raw');
...
xlim([10^4 10^7]);
ylim([0 15000]);
title('Night Average Signal: 2020/09/15');
xlabel('normalized range corrected signal (counts/hour)')
ylabel('Height (m)')
%% SUBPLOT DAY  AVG %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sd = subplot(2,2,4);
a=size(density);
...
xlim([10^4 10^7]);
ylim([0 15000]);
title('Day Average Signal: 2020/09/15');
xlabel('normalized range corrected signal (counts/hour)')
ylabel('Height (m)')
%% SUBPLOT 24H CONTOUR %%%%%%%%%%%%%%%%%%%%%%%%%%%%
cp=subplot(2,2,[1,2]);
...
c = colorbar;
caxis([4 7]);
c.Label.String='log10(normalized range corrected signal (log(counts/hour)))';
c.Label.FontSize = 15;
xlabel('Time (UT)');
ylabel('Height (m)');
a=size(density);
...
%% Adding overlap   
A = importdata('TUB120012_20180924_1024.cfg');


%B = reshape(A.data, 1024, 1);


c = density./B;
%d = bsxfun(@rdivide,densitynew,B);
